...
fn preview_is_keyless_commit_resolves_preserves_and_never_enables_or_mutates_source() {
    ...
    let preview = imports
        .preview(old.path().into(), dest.path().into())
        .unwrap();
    ...
    assert_eq!(
        fs::read(old.path().join("agents/managed-agents.json")).unwrap(),
        before
    );
...
fn changed_source_duplicate_selection_and_credential_failure_do_not_commit() {
    ...
    assert!(imports
        ...
        .is_err());
    fs::write(old.path().join("agents/global-agent-config.json"), b"{}").unwrap();
    assert!(imports
...
fn inline_key_is_verified_and_not_copied_to_native_config() {
    ...
    let dest = tempfile::tempdir().unwrap();
    source(old.path());
    let path = old.path().join("agents/managed-agents.json");
