...
are not loaded. Existing image Markdown is projected as an attachment instead. Spoiler markup
renders as ordinary literal text; there is no reveal interaction. Kind 40008 rich-content diff
rows are not admitted into channel timelines. Custom emoji remain event-local and are not
substituted inside links or code. Authenticated live traffic reconciles through the same session.
Channel creation and composer preview/toolbars are not implemented. Reply counts open a bounded
thread view; attachments are links. Routine freshness labels are not shown; Conversation options → Diagnostics
