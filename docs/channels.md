...
renders as ordinary literal text; there is no reveal interaction. Custom emoji remain event-local
and are not substituted inside links or code. Authenticated live traffic reconciles through the same
session. Channel creation and composer preview/toolbars are not implemented. Reply counts open a
bounded thread view; attachments are links. Routine freshness labels are not shown; Channel Settings → Diagnostics
