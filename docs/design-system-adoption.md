...
| Search | SearchField | Channels, pages and members preserve their query, ref and keyboard handlers. The compact Emoji/GIF picker treatment remains locally owned. |
...
| Tabs | Tabs | Emoji/GIF retains its compact local treatment over Base UI's associated panels and keyboard activation. Workflow mode retains its existing externally owned editor view. |
...
- The Emoji/GIF picker keeps its compact tab and search geometry, including
  Emoji Mart's shadow-root adapter. It reads shared semantic colors, type and the
  host’s keyboard-focus mode. It does not own another appearance preference.
