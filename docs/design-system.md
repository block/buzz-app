...
## Content headers and settings rows
...
`SettingRow` aligns a label and optional description with a caller-owned control.
Pass its `controlId` to the control's `id`, and retain the control's accessible
name. Rows own layout only; services still own preferences and persistence.
