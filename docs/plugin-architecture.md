...
wins; a throwing matcher is skipped. Panels receive `{ target, close, context? }`.
A conversation host may supply `context: { channelId, canOpen, open }` for contextual
panel-to-panel actions. `canOpen` is advisory active-target availability; `open`
re-resolves at click time and returns false after the originating opening, channel,
session or host presentation retires. This is not a global navigation API or an
access grant. Launcher/fallback panels need not have conversation context. A plugin that needs shared data declares `relay` in its
