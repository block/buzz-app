...
Motivating consumer: [PR Beacon’s author names and VIP search](https://github.com/squareup/buzz-plugin-pr-beacon/pull/9),
reviewed at 4863f257ca25b3c2b7c10516854e9c24c7a0a135 (private repository).

## Problem

PR Beacon #9 adds directory names for GitHub authors and person search for VIP
selection. It also adds fetching, indexing, caching, configuration and failure
handling inside PR Beacon. Other plugins needing the same identities would
duplicate those responsibilities. Buzz’s existing profile and identity-naming
services resolve Nostr identities; they do not search an organizational directory
or map GitHub accounts to people.

## Chosen approach

The Directory plugin owns person search, identity lookup, configuration, fetching
and caching, and publishes `directoryV1` through Cordis. PR Beacon consumes it in
an optional child `ctx.inject(["directoryV1"], …)` scope, retaining GitHub names
...
PR Beacon is the first consumer. Organization charts, directory writes, multiple
simultaneous providers and changes to Nostr naming are out of scope. This PR is
...
- `ctx.inject(dependencies, callback)` creates a child scope that starts when its
  ...
  PR Beacon’s top-level dependencies lets the rest of the plugin stay active.
...
PR Beacon keeps its existing GitHub-login VIP format unchanged, including manual
entry; the GitHub authority is implicit in those existing preferences.
...
Illustrative consumer wiring; PR Beacon retains its existing required services:
...
needed. GitHub requests, queue state and saved VIPs remain owned by PR Beacon’s
parent scope.

| Directory state | PR Beacon behavior |
| --- | --- |
...
Directory owns fetching and authenticates independently of PR Beacon. No token
...
Start with the behavior proposed in PR Beacon #9: an in-memory snapshot, 24-hour
freshness, explicit refresh after failure and a 20-second request deadline.
...
PR Beacon also clears transient names/search on its own GitHub disconnect or
account change without clearing the provider cache used by other consumers.
...
username, explicitly selects a result, and PR Beacon saves its GitHub login in
the existing VIP list. Reloading or disabling Directory leaves that VIP usable.
...
1. **Ambiguous query versus conflicting mapping.** Several valid people may match
   ...
   search results, matching PR Beacon #9’s current 1:1 policy. Never infer the
...
1. Agree the contract and authentication mechanism, then implement Directory with
   its configuration UI and PR Beacon’s optional consumption in coordinated PRs.
...
2. Deploy Directory before switching PR Beacon to the shared API. Remove PR
   Beacon’s local directory fetching, cache and configuration; only one directory
...
3. Verify the provider and PR Beacon together in an authenticated desktop host;
   PR Beacon #9 has not established that result. Disable Directory to verify
   fallback. Roll back the consumer migration independently if needed; VIP
   preferences require no conversion.
...
replacement without restarting PR Beacon’s main scope. Control request completion
with deferred operations. A focused desktop check verifies authentication,
...
2. Which host authentication/transport capability will the Directory plugin use?
   Confirm compatibility with the host targeted by PR Beacon #9; do not assume a
   token owned by PR Beacon becomes available to Directory. **Owner:** Directory
   maintainer and Buzz host maintainer. **Needed:** 2026-10-02, before implementation.
...
| PR Beacon uses an optional child scope | Two-way | Local consumer wiring change; required dependency would remove fallback. |
