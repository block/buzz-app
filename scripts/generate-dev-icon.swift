...
// Ported from block/buzz scripts/generate-dev-icon.swift.
...
repeat {
    ...
    fontSize -= 1
} while (lines.count > 2 || lines.contains {
    ($0 as NSString).size(withAttributes: attributes).width > maxBadgeWidth - padding * 4
}) && fontSize > 1
