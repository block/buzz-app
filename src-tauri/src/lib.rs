...
#[tauri::command]
async fn plugin_change<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    manager: tauri::State<'_, PluginManager>,
    action: String,
    id: String,
    ...
    let builder = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let enabled = app
                ...
                .as_ref()
                .ok()
                .and_then(|manager| manager.catalog().ok())
                .is_some_and(|catalog| {
                    catalog.plugins.iter().any(|plugin| {
                        plugin.manifest.id == "buzz.community-compute" && plugin.enabled
                    })
                });
            let compute = ComputeHost::new(
                app.path().app_data_dir().ok(),
                app.config().build.dev_url.as_ref().map(ToString::to_string),
                enabled,
            );
            app.manage(compute.clone());
            let runtime = app
                .path()
                .resource_dir()
                ...
                .unwrap_or_else(|_| {
                    std::env::current_exe()
                        .ok()
                        .and_then(|path| path.parent().map(|parent| parent.join("agent-runtime")))
                        .unwrap_or_default()
                });
