...
import { SettingRow } from "../shared/design-system/ui/SettingRow";
  ...
  return (
    ...
    >
      <Header
        ...
        title="Notifications"
        subtitle="Choices are saved for this account on this device. System permission is separate."
      />
      <div>
        <SettingRow title="Desktop alerts" controlId={`${id}-desktop`}>
          <Switch
            id={`${id}-desktop`}
        ...
        <SettingRow title="Notify while viewing" controlId={`${id}-viewing`}>
          <Switch
            id={`${id}-viewing`}
            aria-label="Notify while viewing"
        ...
        {state.systemManaged ? (
          ...
          <SettingRow
            title="Sound"
            controlId={`${id}-sound`}
            description="Sound uses the system default where supported. Turning it off keeps alerts enabled."
          >
            <Switch
              id={`${id}-sound`}
              aria-label="Sound"
        ...
        >
          <InlineHeader
            ...
            subtitle="Message alerts cover the selected community while Buzz is running. Reading history and reconnecting stay quiet."
          />
          {state.categories.map(({ key, label }) => (
            <SettingRow key={key} title={label} controlId={`${id}-${key}`}>
              <Switch
                id={`${id}-${key}`}
                aria-label={label}
