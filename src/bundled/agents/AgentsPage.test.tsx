...
function records(scope: string, generation: number) {
  ...
  const library = Children.toArray(content.props.children)[1];
  if (!isValidElement<{ children: ReactElement[] }>(library))
    throw new Error("Missing legacy library disclosure");
  const records = Children.toArray(library.props.children)[1];
