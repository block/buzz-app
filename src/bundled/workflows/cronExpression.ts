...
export type CronFieldDefinition = (typeof CRON_FIELD_DEFINITIONS)[number];
...
/**
 * Form mode edits the standard five-field shape only. The relay also accepts
 * six and seven fields (seconds and year); those definitions stay in YAML.
 */
...
function atomError(
  ...
  if (
    ...
    (definition.aliases as readonly string[]).includes(upperAtom)
  ) {
    return null;
  }
  if (!/^\d+$/.test(atom)) {
...
function segmentError(
  ...
  const startError = atomError(rangeStart, definition);
  if (startError) return startError;
  if (rangeEnd === undefined) return null;

  const endError = atomError(rangeEnd, definition);
