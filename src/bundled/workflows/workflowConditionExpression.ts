// Adapted from block/buzz desktop workflow condition helpers.
...
export function defaultConditionOperatorForField(
  ...
  return conditionOperatorsForField(field)[0] ?? "contains";
}
...
/** Form drafts must not silently drop an incomplete condition on save or view switch. */
export function conditionRowError(
...
export function parseConditionExpression(
  ...
  if (empty?.[1] && parseField(empty[1], triggerType)) {
  ...
  if (fn?.[3] && fn[4] !== undefined && parseField(fn[3], triggerType)) {
  ...
  if (
    equality?.[1] &&
    equality[3] !== undefined &&
    parseField(equality[1], triggerType)
  ) {
    const result = {
      field: equality[1],
      operator:
