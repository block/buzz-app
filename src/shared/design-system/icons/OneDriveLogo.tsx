...
/** Microsoft OneDrive brand artwork sourced from SVG Repo. */
export const OneDriveLogoIcon = forwardRef<
  SVGSVGElement,
  SVGProps<SVGSVGElement> & { size?: number | string }
>(function OneDriveLogoIcon(
  { size = "1em", "aria-hidden": ariaHidden = true, ...props },
  ref,
) {
  const id = useId().replaceAll(":", "");
  const mask = `${id}-mask`;
  const gradient = (index: number) => `${id}-gradient-${index}`;
  return (
    // Decorative in Buzz: the adjacent link text owns the accessible name.
    // biome-ignore lint/a11y/noSvgWithoutTitle: decorative SVG is hidden below.
    <svg
      ref={ref}
      width={size}
      height={size}
