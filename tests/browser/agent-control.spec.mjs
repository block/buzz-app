...
test("local controls preserve drafts, confirm operations and distinguish disabled from sleeping", async ({
  ...
  await expect(editor.getByLabel("Replacement for EXAMPLE_TOKEN")).toHaveValue(
    "",
  );
  ...
  await expect(editor.getByRole("alert")).toContainText("newer saved revision");
  await expect(
  ...
  await page.getByRole("button", { name: "Toggle browser-only mode" }).click();
  await closeEditor(page);
...
test("editor renders host choices rather than its own catalog, and tolerates an older host", async ({
  ...
  await page.getByRole("option", { name: "Host harness", exact: true }).click();
  await provider.click();
  await page
...
for (const launch of ["start", "restart"]) {
  for (const lateFailure of [false, true]) {
    test(`${launch} credential wait leaves real Stop buttons usable and ignores late ${lateFailure ? "failure" : "success"}`, async ({
      ...
      await expect(
        other.getByText("Stopped · a later sent mention can start this agent"),
      ).toBeVisible();
      await closeEditor(page);
      await openEditor(page);
      ...
      await page.evaluate(() => window.releaseStop());
      await expect(
        first.getByText("Stopped · a later sent mention can start this agent"),
      ).toBeVisible();
      await expect(
...
for (const changed of ["destination", "source"]) {
  test(`import ${changed} edits invalidate candidates and fence delayed previews`, async ({
    ...
    const expectedDestination =
      changed === "destination" ? "wss://final.example" : "wss://first.example";
    await expect(
...
test("rejected import preview keeps inputs and recovers through Load agents", async ({
  ...
  expect(
    await page.evaluate(() =>
      window.agentControlFixture.calls.some((call) => call.action === "import"),
    ),
  ).toBe(false);
});
