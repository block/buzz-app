...
test("My agents reads the existing library with exact linked keys and session-safe retries", async ({
  ...
  await page.goto("/tests/fixtures/agents.html");
  ...
  await expect(page.getByText(/current Buzz library, read-only/)).toBeVisible();
  ...
  await page
    .getByRole("button", { name: "Refresh agents", exact: true })
    .click();
  await expect(page.getByRole("alert").first()).toContainText("Could not read");
  await page.getByRole("button", { name: "Toggle error", exact: true }).click();
  ...
  await page
    .getByRole("button", { name: "Refresh agents", exact: true })
    .click();
  await expect(page.getByText(/Archive visibility is unknown/)).toHaveCount(0);
  ...
  await page.getByRole("button", { name: "Toggle hold", exact: true }).click();
  await page
    .getByRole("button", { name: "Refresh agents", exact: true })
    .click();
  ...
  await page.getByRole("button", { name: "Community B", exact: true }).click();
  await page
  ...
  await page.getByRole("button", { name: "Community A", exact: true }).click();
  await expect(
    agents.getByRole("heading", { name: "A Brain", exact: true }),
  ).toHaveCount(2);
