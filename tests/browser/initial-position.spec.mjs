...
const sized = test.extend({ historyCounts: { alpha: 20, beta: 20 } });
const short = test.extend({ historyCounts: { alpha: 20, beta: 1 } });
const history = (page) =>
...
sized(
  "cold and warm sidebar entries start at bottom without scrolling",
  async ({ page, app }) => {
    await open(page, app);
