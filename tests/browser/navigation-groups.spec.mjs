...
// Both overlays render through the same real button. A DOM emulator cannot prove
// pointer hit testing, portal dismissal, or focus restoration between them.
test.describe("grouping with unread activity", () => {
  test.use({
    readState: true,
    threadUnread: true,
  ...
  test("group menu preserves the activity popover and current row focus", async ({
    page,
    app,
  }) => {
    await open(page, app);
