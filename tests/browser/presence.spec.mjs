...
test("real same-origin windows queue one publisher and transfer its Web Lock on close", async ({
  ...
  try {
    ...
    await second
      ...
      .click();
    await second
      .getByRole("radio", { name: "Appear offline", exact: true })
      .click();
    await expect(
...
test("avatar choices publish through the existing socket and persist across reload", async ({
  ...
  await avatar.click();
  await account.getByRole("radio", { name: "Away", exact: true }).click();
  await expect(
  ...
  await avatar.click();
  await expect(
    account.getByRole("radio", { name: "Appear offline", exact: true }),
  ).toBeChecked();
  const before = published("online");
  await account.getByRole("radio", { name: "Automatic", exact: true }).click();
  await expect(
    page.getByRole("img", { name: "Your status: Active" }),
  ).toBeVisible();
