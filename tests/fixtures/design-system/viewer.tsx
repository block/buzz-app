...
export function mountViewer(
  productComposer?: ComponentType<{ section?: string }>,
) {
  ...
  const design = createRoute({
    ...
    path: "design",
    component: () => (
      <DesignSystemLayout productUI={Boolean(productComposer)} />
    ),
  });
  const detail = createRoute({
    ...
    component: () => (
      <ComponentDetailPage slug={detail.useParams().component} />
    ),
  });
  const pages = [
    createRoute({
  ...
  const ProductComposer = productComposer;
  const productPages = ProductComposer
    ? [
        createRoute({
          getParentRoute: () => design,
          path: "product-ui/composer",
          component: () => <ProductComposer />,
        }),
        ...["chips", "formatting", "expressions"].map((section) =>
          createRoute({
            getParentRoute: () => design,
            path: `product-ui/composer/${section}`,
            component: () => <ProductComposer section={section} />,
          }),
        ),
      ]
    : [];
  ...
  const router = createRouter({
    routeTree: rootRoute.addChildren([
      design.addChildren([...pages, ...productPages, missing]),
    ]),
