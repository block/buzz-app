...
// Expected values come from block/buzz's scripts/instance-env.sh derivation:
// python3 -c "import hashlib,sys; h=int(hashlib.sha256(sys.argv[1].encode()).hexdigest(), 16); print(10000 + h % 55000)" "$PATH_UNDER_TEST"
const KNOWN = [
  ["/tmp/buzz-worktree-port-fixture", 60252],
  ["/tmp/wörktree ✓", 42704],
];

test("portForPath is deterministic, in range, and matches block/buzz", () => {
  ...
  for (const candidate of ["", "/", "a".repeat(4096), KNOWN[0][0]]) {
    ...
    assert.ok(port >= 10000 && port <= 64999, `${candidate}: ${port}`);
  }
